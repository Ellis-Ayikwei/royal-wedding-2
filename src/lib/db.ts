...
async function seed(db: Client) {
  ...
  if (venueCount === 0) {
    await db.execute({
      sql: `INSERT INTO venue (id, name, ceremonyLocation, receptionLocation, address, latitude, longitude, mapsUrl, image)
            VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [
        "Pot of Gold, East Legon",
        "Pot of Gold — Garden Lawn",
        "Pot of Gold — Grand Hall",
        "Pot of Gold, East Legon, Accra, Ghana",
        null,
        null,
        "",
  ...
  if (eventCount === 0) {
    const items = [
      ...
      {
        title: "The Royal Wedding Ceremony",
        description: "The formal exchange of vows beneath the garden's gold-lit arches.",
